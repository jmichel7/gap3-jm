...
;; February 1993
;;
;;! version 1.981, 20:48 Mon 12 Aug 1996
;;!
;;
;;
...
;;! ----------------------------------------------------------------------
;;! v1.981 -
;;! * Allowed for variable names starting with _ in gap-add-local-variables.
;;!   Also set word syntax for _.
;;! v1.98 -
;:! * Made gap-add-local-variables a little more intelligent --- now it
;;!   will ignore assignments to fields of a record (as it should!).
;;! v1.97 -
;;! * Added my own comment filling function, gap-wrap-comment. This is
;;!   called when not in the minor gin-mode (I would remove the gin-mode
;;!   feature if I knew whether no-one used it).
;;! v1.96 -
...
(if gap-syntax-table ()
  ...
  (modify-syntax-entry ?_ "w" gap-syntax-table) ;; . is part of identifiers
...
(defun gap-wrap-comment nil
  (interactive)
  (save-excursion
    (beginning-of-line)
    (looking-at "^ *#+ *")
    (let* ((filler (buffer-substring (match-beginning 0) (match-end 0)))
           (pat (concat filler "[^ \n\t]"))
           (fill-column (- fill-column (length filler)))
           start end)
      (message (concat "<" filler ">"))
      (while (and (< (point) (point-max)) (looking-at pat))
        (forward-line 1))
      (forward-line -1) (end-of-line) (setq end (point)) (beginning-of-line)
      (while (and (> (point) 0) (looking-at pat))
        (forward-line -1))
      (forward-line 1) (setq start (point))
      (narrow-to-region start end)
      (replace-regexp (concat "^" filler) "")
      (fill-paragraph nil)
      (goto-char (point-max))
      (delete-char -1)
      (goto-char (point-min))
      (replace-regexp "^" filler)
      (widen))))
  ...
  (let (p1 p2
    ...
    (save-excursion
      ...
      (while (gap-searcher 're-search-forward 
        ...
        (cond ((looking-at "\\(^\\|;\\) *\\([a-z_][a-z0-9_]*\\) *:= *")
               (setq name (buffer-substring (match-beginning 2) (match-end 2)))
               (goto-char (match-end 0))
               (if (gap-point-in-brackets (match-beginning 1))
                   (setq name nil)
              ...
              ((looking-at "\\(^\\|;\\) *for +\\([a-z_][a-z0-9_]*\\) +in\\>")
               (setq name (buffer-substring (match-beginning 2) (match-end 2)))
               (goto-char (match-end 0)))
              (t (error "gap-insert-local-variables incorrect code!")))
        (if (and name (not (memberequal name names)))
...
(defun gap-point-in-brackets (pos)
  (save-excursion
    (let (this-beg)
