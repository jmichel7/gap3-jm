...
##
#A  @(#)$Id: field.g,v 1.1.1.1 1996/12/11 12:37:51 werner Exp $
##
...
##
#H  $Log: field.g,v $
#H  JM 2015/02/25 moved from chevie patch to FieldOps.\= and FieldOps.String
...
##
## JM 2015/02/25 moved from chevie/lib/patch version which special-cases
##               Cyclotomics to take in account they have no generators

FieldOps.\=:=function (F, G) local  isEql;
  if IsField(F) and IsFinite(F)  then
    if IsField(G) and IsFinite(G)  then
      isEql:=Size(F)=Size(G) and ForAll(F.generators, g->g in G)
			     and ForAll(G.generators, g->g in F);
    elif IsField(G) then isEql:=false;
    else isEql:=DomainOps.\=(F, G);
    fi;
  elif IsField(F) then
    if IsField(G) and IsFinite(G) then isEql:=false;
    elif IsField(G) then
      if not IsBound(F.generators) then
        if not IsBound(G.generators) then
          if F.name="Cyclotomics" and G.name="Cyclotomics" then 
	    return true;
	  else Error("unforeseen case");
	  fi;
	else return false;
	fi;
      elif not IsBound(G.generators) then return false;
      fi;
      isEql:=ForAll(F.generators, g->g in G) and 
	     ForAll(G.generators, g->g in F);
    else isEql:=DomainOps.\=(F, G);
    fi;
  else isEql:=DomainOps.\=(F, G);
  fi;
...
#############################################################################
##
#F  FieldOps.String( <F> ) . . . . . . . . . . . . . . . . . String for field
##
FieldOps.String:=function(F)local res;
  if IsBound(F.name) then return F.name;fi;
  res:=SPrint("Field( ",Join(F.generators)," )");
  if IsBound(F.field) then PrintToString(res, "/", F.field );fi;
  return res;
end;
...
##
FieldOps.Print:=function(F)Print(String(F));end;
