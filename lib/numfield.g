...
##
#A  @(#)$Id: numfield.g,v 1.3 1998/02/27 10:22:40 gap Exp $
##
...
##
#H  $Log: numfield.g,v $
#H  JM 2015/02/25 moved from chevie NumberFieldOps.String and
#H                CyclotomicFieldOps.String
#H  Revision 1.3  1998/02/27 10:22:40  gap
#H  Various fixes by Thomas.
#H
#H  Revision 1.2  1997/01/15 10:35:27  gap
#H  fixed dangerous bug in 'Intersection' for abelian number fields
#H      (found during tests with GAP-4 ...)
#H
#H  Revision 1.1.1.1  1996/12/11 12:37:44  werner
...
#############################################################################
##
#F  NumberFieldOps.String( <F> ) . . . . . . . . . .  String for number field
##
NumberFieldOps.String:=function(F)local res;
  if IsBound(F.name) then return F.name;fi;
  if IsBound(F.basechangemat) then 
    return SPrint("NF(",F.field,",",F.base,")");fi;
  res:=SPrint("NF(",NofCyc(F.generators),",",F.stabilizer,")");
  if F.field<>0 then PrintToString(res,"/",F.field);fi;
  return res;
end;
...
#############################################################################
##
#F  CyclotomicFieldOps.String( <F> ) . . . . . String for a cyclotomic field
##
CyclotomicFieldOps.String:=function(F)local res;
  if IsBound( F.name )  then return F.name;fi;
  if IsBound(F.basechangemat) then 
    return SPrint("CF( ",F.field,",",F.base,")");fi;
  res:=SPrint( "CF(", NofCyc( F.generators ), ")" );
  if F.field<>0 and F.field<>Rationals then PrintToString(res,"/",F.field);fi;
  return res;
end;
...
#############################################################################
#F  CyclotomicsOps.PolynomialRing( <R> )  . . . . . . .  full polynomial ring
##
CyclotomicsOps.PolynomialRing := function( R )
    return CyclotomicsPolynomials;
end;

#############################################################################
##
#F  CyclotomicsOps.Polynomial( <R>, <coeffs>, <val> ) . . polynomial over <R>
##
CyclotomicsOps.Polynomial := function( R, coeffs, val )
    coeffs := ShallowCopy(coeffs);
    val    := val + NormalizeCoeffs(coeffs); # normalize <coeffs>
    return CyclotomicsPolynomialOps.New(R,coeffs,val);
end;

#############################################################################
##
#F  CyclotomicsOps.FastPolynomial(<R>, <coeffs>, <val>) . polynomial over <R>
##
##  This function will *not* copy <coeffs> and thus destroy <coeffs>.
##
CyclotomicsOps.FastPolynomial := function( R, coeffs, val )
    val := val + NormalizeCoeffs(coeffs); # normalize <coeffs>
    return CyclotomicsPolynomialOps.New(R,coeffs,val);
end;
