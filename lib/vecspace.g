...
##
#A  @(#)$Id: vecspace.g,v 1.1.1.1 1996/12/11 12:37:58 werner Exp $
##
...
##
Coefficients := function(arg)local V,v,cf;
    V:=arg[1];
    if IsList(V) then cf:=CoefficientsInt(V,v);
    elif IsRec(V) and IsBound(V.operations.Coefficients) then
        cf:=ApplyFunc(V.operations.Coefficients,arg);
    else
        Error( "sorry, <V> has no method for coefficients" );
    fi;
    return cf;
end;
