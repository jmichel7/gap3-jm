...
##
#A  @(#)$Id: domain.g,v 1.1.1.1 1996/12/11 12:37:58 werner Exp $
##
...
##
#H  $Log: domain.g,v $
#H  Revision 1.1.1.1  1996/12/11 12:37:58  werner
...
##
Elements:=function(arg)local D;
  D:=arg[1];
  if IsSet( D ) then return D;
  elif IsRec(D) then
      if IsBound(D.elements ) then return D.elements;
      elif IsBound(D.operations) and IsBound(D.operations.Elements) then
        D.elements := ApplyFunc(D.operations.Elements,arg);
        return D.elements;
      fi;
  else Error("<D> must be a set or have a method for Elements");
  fi;
end;
