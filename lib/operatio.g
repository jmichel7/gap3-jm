...
##
#A  @(#)$Id: operatio.g,v 1.1.1.1 1996/12/11 12:37:43 werner Exp $
##
...
##
#H  $Log: operatio.g,v $
#H  Revision 1.1.1.1  1996/12/11 12:37:43  werner
...
##
Cycles := function ( arg ) local   cycs,  D; D:=arg[1];

    if Length(arg) = 2  then
        return Domain([D]).operations.Cycles( arg[1], arg[2], OnPoints );
    elif Length(arg) = 3  then
        return Domain([D]).operations.Cycles( arg[1], arg[2], arg[3] );
    elif Length(arg) = 1  then
      if IsPerm(D) then return Cycles(D,MovedPoints(D));
      elif IsRec(D) and IsBound(D.operations.Cycles) then
        return D.operations.Cycles(D);
      else  Error(D," has no method for Cycles");
      fi;
    else
...
#############################################################################
##
#F  MovedPoints( <g> )  . . . . . . moved points by an element or a group
##
#F

MovedPoints := function ( x ) local mp, i;
  if IsPerm(x) then
    if x = () then return [ ];
    else
      mp := [ ];
      for i in [1..LargestMovedPointPerm(x)] do
	if i^x <> i then Add(mp, i); fi;
      od;
      IsSet(mp);
      return mp;
    fi;
  elif IsRec(x) and IsBound(x.operations) and IsBound(x.operations.MovedPoints)
  then return x.operations.MovedPoints(x);
  fi;
end;

#############################################################################
##
#F  NrMovedPoints( <g> )  . . . . Nr of moved points by an element or a group
##
#F
NrMovedPoints:= function ( x ) local mp, i;
  if IsPerm(x) then
    if x = () then return 0;
    else
      mp := 0;
      for i in [1..LargestMovedPointPerm(x)] do
	if i^x <> i then mp := mp + 1; fi;
      od;
      return mp;
    fi;
  elif IsRec(x) and IsBound(x.operations) and IsBound(x.operations.NrMovedPoints)
  then return x.operations.NrMovedPoints(x);
  fi;
end;
...
IsRegular := function ( arg )
    ...
    # dispatch to the appropriate function
    if Length(arg) = 1  then # to enable package xmod
        if not IsRec(arg[1]) then return false;fi;
        if IsBound( arg[1].isRegular) then return arg[1].isRegular;fi;
        isReg:= arg[1].operations.IsRegular( arg[1] );
    elif Length(arg) = 2  then
