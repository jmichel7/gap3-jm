...
##
#A  @(#)$Id: matrix.g,v 1.1.1.1 1996/12/11 12:37:45 werner Exp $
##
...
##
#H  $Log: matrix.g,v $
#H  Revision 1.1.1.1  1996/12/11 12:37:45  werner
...
##
NullMat := function ( arg ) local   null, m, n, zero, row, i, k;

    m:= arg[1];
    if   Length(arg)=1   then n:= m; else n:=arg[2];fi;
    if   Length(arg)<3   then zero := 0;
    elif Length(arg)>3   then Error("usage: NullMat( <m>, <n> [, <F>] )");
    elif IsField(arg[3]) then zero := arg[3].zero;
    elif IsRing(arg[3])  then zero := arg[3].zero;
    else                      zero := arg[3]-arg[3];
    fi;
...
#############################################################################
##
#F  IsDiagonalMat( <mat> ) . . . . . .  true iff <mat> is square and diagonal
##  
IsDiagonalMat:=function(M)local i, j, l;l:=Length(M);
  if l>0 and Length(M[1])<>l then return false;fi;
  for i in [1..l] do
...
#############################################################################
##
#F  IsUpperTriangularMat( <M> ) . . . . . true iff <M> is upper triangular
##  
IsUpperTriangularMat:=function(M)local n;n:=Length(M);
  return ForAll([2..n],i->ForAll([1..i-1],j->M[i][j]=0*M[i][j]));
end;  
...
##  scalars.  DiagonalMat( <v> )  where v is  a list of  scalars is like
##  DiagonalMat(v1,..vn)
##
DiagonalMat:=function(arg)local res,fr,fc,r,c,m;
  if Length(arg)=0 then return [];
  elif Length(arg)=1 and IsList(arg[1]) and Length(arg[1])>0 and not
    IsList(arg[1][1]) then arg:=arg[1];
  fi;
  fr:=function(m)if IsList(m) then return Length(m);else return 1;fi;end;
  fc:=function(m)if not IsList(m) then return 1;
    elif Length(m)=0 then return 0; else return Length(m[1]);fi;end;
  res:=Flat(arg);if res=[] then return res;fi;
  res:=res[1]*NullMat(Sum(arg,fr),Sum(arg,fc));
  r:=0;c:=0;
  for m in arg do
    if IsList(m) then res{r+[1..fr(m)]}{c+[1..fc(m)]}:=m;
...
#############################################################################
##
#F  TriangulizeMat( <mat> ) . . . . . bring matrix <mat> to upper triangular
##  form.  Returns  the  indices  of  the  lines  of  the  original  matrix
##  corresponding  to the lines retained in the triangulized matrix.
##
TriangulizeMat := function (mat)local  m, n, rk, j, k, row, zero,inds;
  InfoMatrix1("#I  TriangulizeMat called\n");
  if mat =[] then return[];fi;
  m := Length(mat); n := Length(mat[1]); zero := 0*mat[1][1];
  rk := 0;inds:=[1..m];
  InfoMatrix2("#I    nonzero columns: \c");
  for k  in [1..n]  do
      # find a nonzero entry in mat{[rk+1..m]}[k]
      j := rk + 1; while j <= m and mat[j][k] = zero do j := j + 1;  od;
      if j<=m then # if there is a nonzero entry
        InfoMatrix2(k," \c");
	rk:=rk+1; # increment the rank
	# make its row the current row and normalize it
	row := mat[j];  mat[j] := mat[rk];  mat[rk] := row[k]^-1 * row;
	inds{[j,rk]}:=inds{[rk,j]};
	# clear all entries in this column
	for j  in [1..m] do
	  if  rk <> j  and mat[j][k] <> zero  then
	      mat[j] := mat[j] - mat[j][k] * mat[rk];
	  fi;
	od;
      fi;
  od;
...
MatricesOps.Group := function ( Matrices, gens, id )
    ...
    # check that the generators are all of the same size and invertible
    d := Length(id);
    for g  in gens  do
