...
##
#A  @(#)$Id: classfun.g,v 1.1.1.1 1996/12/11 12:37:55 werner Exp $
##
...
##
#H  $Log: classfun.g,v $
#H  JM 2015/02/25 ported from chevie improved version of Character
#H  Revision 1.1.1.1  1996/12/11 12:37:55  werner
...
#V  CharacterOps
##
CharacterOps := OperationsRecord( "CharacterOps",VirtualCharacterOps );

#############################################################################
##
#F  Character( <obj> )
##  JM 2015/02/25 ported from chevie: any obj instead of <repres>
#F  Character( <G>, <values> )
#F  Character( <G>, <chi> )
##
Character:=function(arg)local G, chi;
  if Length( arg ) = 1 and IsRec( arg[1] ) then
    return arg[1].operations.Character( arg[1] );
  elif Length(arg)=2 then
    chi:=ApplyFunc(ClassFunction,arg);
    chi.isVirtualCharacter := true;
    chi.isCharacter        := true;
    chi.operations         := CharacterOps;
    return chi;
  else Error( "usage: Character(<obj>) or Character(<G>,<values>)" );
  fi;
end;
...
CharacterString := function( chi )
    return Concatenation( "of degree ", String( chi.values[1] ),
