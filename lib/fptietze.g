...
##
#A  @(#)$Id: fptietze.g,v 1.4 2000/05/12 09:48:37 sal Exp $
##
...
##
#H  $Log: fptietze.g,v $
#H  Revision 1.4  2000/05/12 09:48:37  sal
#H  Volkmar Felsch's fix for the segfault that Isabel reported (GAP 3)	SL
#H
#H  Revision 1.3  1997/06/25 11:36:57  gap
#H  Fixes by V. Felsch. AH
#H
#H  Revision 1.2  1997/01/14 15:18:00  gap
#H  vfelsch added 'PresentationViacosetTable', the 'protected' mechanism, and
#H  the tracing of generator images through Tietze transformations
#H
...
##
PresentationOps := OperationsRecord("PresentationOps");
...
##
PresentationOps.AddGenerator := function ( arg )
...
#############################################################################
##
#F  TzRenumberGenerators( <Tietze record>, <sort list> ) . . . . Renumber the
#F               generators of T according to L
##
##  'TzRenumberGenerators' renumbers the generators of the given presentation
##  record T according to the given sort list L.
##
TzRenumberGenerators := function ( T, L )

    local comps, gens, i, invs, invsort, j, new, numgens, numgens1, numrels,
          oldcomps, oldgens, oldinvs, perm, rels, tietze, tree;

    if T.printLevel >= 3 then
        Print( "#I  renumbering the Tietze generators\n" );
    fi;

    # ensure that there are no redundant generators.
    TzRemoveGenerators( T );
    tietze := T.tietze;
    numgens := tietze[TZ_NUMGENS];

    # check the given sort list.
    if not IsList( L ) then
        Error( "second argument must be a list" );
    fi;
    if Length( L ) <> numgens or Set( L ) <> [ 1 .. numgens ] then
...
Print( "rels = ", rels, "\n" );
    for i in [ 1 .. numgens ] do
        ...
        gens[i] := oldgens[j];
        comps[i] := oldcomps[j];
        if oldinvs[numgens1+j] = j then
            invs[numgens1+i] := i;
        else
            invs[numgens1+i] := -i;
        fi;
        new[numgens1+i] := invsort[i];
        new[numgens1-i] := -invsort[i];
    od;
Print( "gens = ", gens, "\n" );
Print( "comps = ", comps, "\n" );
...
Print( "new = ", new, "\n" );
    ...
    # update the relators.
    for i in [ 1 .. numrels ] do
        rels[i] := List( rels[i], j -> new[numgens1+j] );
    od;
Print( "rels = ", rels, "\n" );

    # if there is a tree, update its entries.
    if IsBound( T.tree ) then
        tree := T.tree;
        tree[TR_TREENUMS] := Permuted( tree[TR_TREENUMS], perm );
    fi;

    # if there are generator images, update them.
    if IsBound( T.imagesOldGens ) then
Print( "preimages = ", T.preImagesNewGens, "\n" );
Print( "images = ", T.imagesOldGens, "\n" );
        for i in [ 1 .. Length( T.imagesOldGens ) ] do
            T.imagesOldGens[i] :=
                List( T.imagesOldGens[i], j -> new[numgens1+j] );
        od;
        T.preImagesNewGens := Permuted( T.preImagesNewGens, perm );
Print( "preimages = ", T.preImagesNewGens, "\n" );
Print( "images = ", T.imagesOldGens, "\n" );
    fi;
