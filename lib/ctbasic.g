...
#A                                                           & Goetz Pfeiffer
##
#A  @(#)$Id: ctbasic.g,v 1.5 1997/09/11 09:48:31 gap Exp $
##
...
##
#H  $Log: ctbasic.g,v $
#H  Revision 1.5  1997/09/11 09:48:31  gap
#H  Fix for 'RealClassesCT' (by Thomas). AH
#H
#H  Revision 1.4  1997/01/25 18:28:40  werner
#H  Fixed a couple of revision comments.
#H
#H  Revision 1.3  1997/01/23 10:27:54  sal
#H  Small change suggested by Goetz to improve the printing of some character
#H  tables.
#H
#H      SL	23/1/97
#H
#H  Revision 1.2  1997/01/20  08:36:30  gap
#H  fixed serious bug, namely an infinite loop in 'DisplayCharTable'
#H     for the case that the line length is smaller than the required space
#H     for a single column
#H     (reported by Meinolf Geck as a problem for Hecke tables, but this
#H     was a general bug)
#H
#H  Revision 1.1.1.1  1996/12/11 12:37:54  werner
...
CharTable := function( arg )
    ...
    if Length(arg) < 1 or not
         ( IsString(arg[1]) or IsRec(arg[1]) or IsCharTable(arg[1]) ) then
      Error( "usage: CharTable( <obj> ) resp. CharTable( <tblname> )\n",
    ...
    if   IsCharTable( arg[1] ) then
      ...
      elif IsBound(arg[1].operations) and 
           IsBound( arg[1].operations.CharTable ) then
        arg[1].charTable:= arg[1].operations.CharTable( arg[1] );
        return arg[1].charTable;
      else
        Error( "Sorry, I don't know how to ",
               "construct the character table of ",arg[1],"\n");
      fi;
