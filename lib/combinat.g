...
##
#A  @(#)$Id: combinat.g,v 1.1.1.1 1996/12/11 12:37:55 werner Exp $
##
...
##
AssociatedPartition := function(pi) local i, j, mu;
   if pi=[] then return [];fi;
   mu:=[1..pi[1]]*0;
   for i in pi do for j in [1..i] do mu[j]:= mu[j]+1; od; od;
   return mu;
end;

###########################################################################
#U Dominates(mu,nu) . . . . . . . . . . . . . . . . whether mu dominates nu
##
...
Dominates:=function(mu, nu) local i, m, n, lnu, lmu;
  if nu=mu then return true; elif nu>mu then return false; fi;
...
NrPartitionTuples:=function(n,k)local res,l,r;res:=0;
  for l in [0..k] do
    r:=Binomial(k,l);
    res:=res+Sum(Partitions(n,l),
         a->r*NrArrangements(a,l)*Product(a,NrPartitions));
  od;
  return res;
end;
