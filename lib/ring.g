...
##
#A  @(#)$Id: ring.g,v 1.2 1998/02/27 10:26:15 gap Exp $
##
...
##
#H  $Log: ring.g,v $
#H  Revision 1.2  1998/02/27 10:26:15  gap
#H  Force units to be a set.
#H
#H  Revision 1.1.1.1  1996/12/11 12:37:39  werner
...
##
Gcd := function ( arg ) local   R, ns, i, gcd;

    # get and check the arguments (what a pain)
    if Length(arg) = 0  then Error("usage: Gcd( [<R>,] <r1>, <r2>... )");fi;
    if IsRing(arg[1]) then R:=arg[1];arg:=arg{[2..Length(arg)]};fi;
    if Length(arg) = 1  and IsList(arg[1]) then ns := arg[1];
    else ns:=arg;
    fi;
    if Length(ns)=0  then Error("usage: Gcd( [<R>,] <r1>, <r2>... )");fi;
    if not IsBound(R) then R:=DefaultRing(ns);
    elif not ForAll(ns,n->n in R) then Error("<r> must be an element of <R>");
    fi;
    if not IsEuclideanRing(R) then Error("<R> must be a Euclidean ring");fi;
