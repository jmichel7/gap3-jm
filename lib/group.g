...
##
#H  @(#)$Id: group.g,v 1.3 1998/03/25 15:31:40 gap Exp $
##
...
##
#H  $Log: group.g,v $
#H  JM 2015/02/25 ported from chevie GroupOps.String
#H  Revision 1.3  1998/03/25 15:31:40  gap
#H  Kratzer error.
#H
#H  Revision 1.2  1997/01/22 14:04:47  gap
#H  PErmitted 'AbelianInvariants' for nonabelian groups (applies on G/G')
#H
#H  Revision 1.1.1.1  1996/12/11 12:37:49  werner
...
GroupString := function ( G, name )
    if IsBound( G.name )  then
        ...
        if IsBound( G.size )  then
            return ConcatenationString(
                        "<", name, "> ",
                        "(", String(Length(G.generators)), " gens, ",
                        "size ", String(G.size), ")"
                   );
        else
            return ConcatenationString(
...
Group := function ( arg )
    ...
    elif Length( arg ) = 2 and IsList( arg[1] ) and not (IsMat(arg[1])
      and IsMat(arg[2])) then
        gens := arg[1];
        id   := arg[2];
...
##
Parent := function ( arg ) local   G;

    # check the number of arguments
    if Length( arg ) = 0  then
        Error( "usage: Parent( <G>, ... )" );
    fi;
    if IsSet(arg[1]) then return arg[1]; fi; #for Glissando package
...
##
GroupOps.Print:=function(G)Print(String(G));end;
# does not work for word generators
...
#F  GroupOps.Print( <G> ) . . . . . . . . . . . . . . .  pretty print a group
##
GroupOps.Print := function ( G ) local   i;
  if IsBound( G.name )  then Print( G.name );
  elif not IsBound( G.parent )  then
    # if the group is not trivial the identity need not be printed
    if 0 < Length(G.generators)  then
	Print( "Group( ");
	for i in [1..Length(G.generators)-1] do Print(G.generators[i],", ");od;
	Print( G.generators[Length(G.generators)]," )");

    # if the group is trivial print it as 'Group( <id> )'
    else Print( "Group( ", G.identity, " )" );
    fi;

  # if the group is a subgroup print it as 'Subgroup(...)'
  else Print( "Subgroup( ", G.parent, ", ", G.generators, " )" );
  fi;
end;

#############################################################################
##
#F  GroupOps.String( <G> ) . . . . . . . . . . . . . . .  String for groups
##
#   JM 2015/02/25 ported from chevie
GroupOps.String:=function(G)local res,n,i;
  if IsBound(G.name) then return G.name;
  elif IsBound(G.parent) then
    res:="Subgroup( "; Append(res,String(G.parent));
    Add(res,','); Append(res,String(G.generators));
  else 
    res:="Group(";n:=Length(G.generators);
    if n=0 then Append(res,String(G.identity));
    else for i in [1..n] do Append(res,String(G.generators[i]));
      if i<>n then Add(res,',');fi;od;
    fi;
  fi;
  Add(res,')');
  return String(res);
