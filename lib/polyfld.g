...
##
#A  @(#)$Id: polyfld.g,v 1.1.1.1 1996/12/11 12:37:41 werner Exp $
##
...
##
#H   $Log: polyfld.g,v $
#H  jm 2015/02/25 use vector code for QuotientRemainder
...
FieldPolynomialRingOps.QuotientRemainder := function ( R, f, g )
    ...
    if f.valuation < 0  then Error( "<f> must not be laurent polynomial" ); fi;
    if g.valuation < 0  then Error( "<g> must not be laurent polynomial" ); fi;

    if 0 = Length(g.coefficients)  then Error( "<g> must not be zero" ); fi;
    if 0 = Length(f.coefficients)  then return f; fi;
    ...
    # Divide <f> by <g>
    q := []; n := Length(g); m := Length(f) - n;
    for i  in [m,m-1..0]  do
      c:=f[i+n]/g[n]; k:=[i+1..i+n]; f{k}:=f{k}-c*g; q[i+1]:=c;
    od;
...
##
FieldLaurentPolynomialRingOps.StandardAssociate := function( R, f )local  l;
  l := f.coefficients[Length(f.coefficients)];
  return R.baseRing.operations.FastPolynomial(R.baseRing,f.coefficients*l^-1,0);
end;
