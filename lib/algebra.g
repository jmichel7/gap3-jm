...
##
#A  @(#)$Id: algebra.g,v 1.1.1.1 1996/12/11 12:37:56 werner Exp $
##
...
AlgebraString := function( A, name )
    if IsBound( A.name )  then
      if IsBound( A.size )  then
        return Concatenation( A.name, " (size ", String( A.size ), ")" );
      else
        return A.name;
      fi;
    else
      if IsBound( A.size )  then
        return Concatenation( "<", name, "> (",
                               String(Length(A.generators)),
                               " gens, size ", String(A.size), ")" );
      else
        return Concatenation( "<", name, "> (",
                              String(Length(A.generators)), " gens)" );
      fi;
    fi;
    end;
