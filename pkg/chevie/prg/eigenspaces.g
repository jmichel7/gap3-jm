...
# InclusionGens(W); if this fails all roots are tried.
#
...
# Print("PermRootOps.RelativeGroup ",arg," called \n");
  ...
  if Length(arg)<3 then
    indices:=Filtered(InclusionGens(W),x->not Reflection(Parent(W),x) in L);
  else
    indices:=arg[3];
  fi;
...
#  Print(res.roots,res.simpleCoroots,"\n");
  ...
  if N=Size(R) then 
    Inherit(res,R);
#   res.relativeIndices:=res.relativeIndices{List(InclusionGens(R),x->
...
PermRootOps.StandardParabolic:=function(W,H)local wr,I,i,w,l,hr,w1,a,try;
  wr:=InclusionGens(W); hr:=InclusionGens(H);
 ...
 # first occurence of each reflection in W.reflections
  ...
  while Length(cl)>0 do
    ...
    if HF=false then 
      ...
      P:=[P,OnTuples(InclusionGens(H),P)];
      if P[1]<>() or P[2]<>InclusionGens(H) then
