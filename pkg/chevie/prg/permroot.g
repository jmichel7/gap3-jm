...
# reflections. This function classifies W (returns a type record) using:
#  1- r:=rank
#  2- s:=size
#  3- o:=the set of divisors of the orders of distinguished reflections
#  4- sumcodeg:=Sum(reflection codegrees)=nb. distinguished reflections-rank
#
# G(de,e,r) has:
#  s=r!(de)^r/e, o={2} union divisors(d), sumcodeg=edr(r-1)/2 (-r if d=1)
#
# One  can  check  that  (r,s,o)  are  sufficient  to determine a G(de,e,r)
# excepted  for ambiguity between  G(4e,4e,2) and G(2e,e,2),  which is then
# resolved by Sum(codeg) [remaining ambiguity for e=1 is an isomorphism].
...
#  G17/G(120,24,2)
#  G21/G(120,40,2)
# They are resolved by Sum(codeg).
PermRootOps.TypeIrred:=function(W)local s,prim,r,o,de,ST,sumcodeg,SumCoDegrees;
  prim:=[
  rec(ST:=4,       s:=24,       r:=2, sumcodeg:=2,  o:=[3]), 
  rec(ST:=5,       s:=72,       r:=2, sumcodeg:=6,  o:=[3]), 
  rec(ST:=6,       s:=48,       r:=2, sumcodeg:=8,  o:=[2, 3]), 
  rec(ST:=7,       s:=144,      r:=2, sumcodeg:=12, o:=[2, 3]), 
  rec(ST:=8,       s:=96,       r:=2, sumcodeg:=4,  o:=[2, 4]), 
  rec(ST:=9,       s:=192,      r:=2, sumcodeg:=16, o:=[2, 4]), 
  rec(ST:=10,      s:=288,      r:=2, sumcodeg:=12, o:=[2, 3, 4]), 
  rec(ST:=11,      s:=576,      r:=2, sumcodeg:=24, o:=[2, 3, 4]), 
  rec(ST:=12,      s:=48,       r:=2, sumcodeg:=10, o:=[2]), 
  rec(ST:=13,      s:=96,       r:=2, sumcodeg:=16, o:=[2]), 
  rec(ST:=14,      s:=144,      r:=2, sumcodeg:=18, o:=[2, 3]), 
  rec(ST:=15,      s:=288,      r:=2, sumcodeg:=24, o:=[2, 3]), 
  rec(ST:=16,      s:=600,      r:=2, sumcodeg:=10, o:=[5]), 
  rec(ST:=17,      s:=1200,     r:=2, sumcodeg:=40, o:=[2, 5]), 
  rec(ST:=18,      s:=1800,     r:=2, sumcodeg:=30, o:=[3, 5]), 
  rec(ST:=19,      s:=3600,     r:=2, sumcodeg:=60, o:=[2, 3, 5]), 
  rec(ST:=20,      s:=360,      r:=2, sumcodeg:=18, o:=[3]), 
  rec(ST:=21,      s:=720,      r:=2, sumcodeg:=48, o:=[2, 3]), 
  rec(ST:=22,      s:=240,      r:=2, sumcodeg:=28, o:=[2]), 
  rec(series:="H", s:=120,      r:=3, sumcodeg:=12, o:=[2]), 
  rec(ST:=24,      s:=336,      r:=3, sumcodeg:=18, o:=[2]), 
  rec(ST:=25,      s:=648,      r:=3, sumcodeg:=21, o:=[3]), 
  rec(ST:=26,      s:=1296,     r:=3, sumcodeg:=30, o:=[2, 3]), 
  rec(ST:=27,      s:=2160,     r:=3, sumcodeg:=42, o:=[2]), 
  rec(series:="F", s:=1152,     r:=4, sumcodeg:=20, o:=[2]), 
  rec(ST:=29,      s:=7680,     r:=4, sumcodeg:=36, o:=[2]), 
  rec(series:="H", s:=14400,    r:=4, sumcodeg:=56, o:=[2]), 
  rec(ST:=31,      s:=46080,    r:=4, sumcodeg:=56, o:=[2]), 
  rec(ST:=32,      s:=155520,   r:=4, sumcodeg:=76, o:=[3]), 
  rec(ST:=33,      s:=51840,    r:=5, sumcodeg:=40, o:=[2]), 
  rec(ST:=34,      s:=39191040, r:=6, sumcodeg:=120, o:=[2]), 
  rec(series:="E", s:=51840,    r:=6, sumcodeg:=30, o:=[2]), 
  rec(series:="E", s:=2903040,  r:=7, sumcodeg:=56, o:=[2]), 
  rec(series:="E", s:=696729600, r:=8, sumcodeg:=112, o:=[2])];
  r:=RankMat(W.roots{W.generatingReflections});
  SumCoDegrees:=W->Length(Reflections(W))-r;
  s:=Size(W);
  # Now find de=possible [d,e] for given  s and r
  de:=s/Factorial(r);
  if not IsInt(de) then de:=[];
  elif de=r+1 then return rec(series:="A",rank:=r);
  elif r=1 then return rec(series:="ST",p:=de,q:=1,rank:=1);
  else de:=List(Cartesian(List(Collected(Factors(de)),function(x)local u,a,res;
      ...
      return res;
    end)),p->rec(d:=Product(p,x->x[1]),e:=Product(p,x->x[2])));
  fi;
  o:=Union(List(W.OrdersGeneratingReflections,DivisorsInt)); # now use o
  de:=Filtered(de,x->o=Union([2],DivisorsInt(x.d)));
  ST:=Filtered(prim,f->r=f.r and s=f.s and o{[2..Length(o)]}=f.o);
  if Length(de)>1 then # G(2e,e,2),G(4e,4e,2) or G12,G13,G22
    if Length(de)<>2 then Error("theory");fi;
    Sort(de);
    sumcodeg:=SumCoDegrees(W);
    if sumcodeg=de[2].d*de[2].e then de:=[de[2]];
    elif sumcodeg=de[1].d*de[1].e-2 then de:=[de[1]];
    elif Length(ST)<>1 or sumcodeg<>ST[1].sumcodeg then Error("theory");
  ...
  # [G(120,40,2),G21]
    if not IsBound(sumcodeg) then sumcodeg:=SumCoDegrees(W);fi;
    ST:=Filtered(ST,i->i.sumcodeg=sumcodeg);
    if Length(ST)>1 then Error("theory");
    elif Length(ST)>0 then 
      if sumcodeg=de[1].d*de[1].e then Error("theory");fi;
