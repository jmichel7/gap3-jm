...
static BOOLEAN matrix01AutoProperty(const Permutation *const s )
{
   return isMatrix01Isomorphism( MM, MM, s, checkMonomialProperty);
}


static BOOLEAN matrix01IsoProperty(const Permutation *const s )
{
   return isMatrix01Isomorphism( MM_L, MM_R, s, checkMonomialProperty);
}


static BOOLEAN codeAutoProperty(const Permutation *const s )
{
   return isCodeIsomorphism( CC, CC, s);
}


static BOOLEAN codeIsoProperty(const Permutation *const s )
{
   return isCodeIsomorphism( CC_L, CC_R, s);
}

static void informMatrixIso(Permutation *perm);

static void informMatrixMonIso(Permutation *perm);

static void informCodeMonIso(Permutation *perm);
