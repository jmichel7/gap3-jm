...
##
#H  $Log: codeman.g,v $
#H  Revision 1.1  1997/01/20 15:15:06  werner
...
CodeOps.ExtendedCode := function (Cold, nrcolumns ) 
    ...
    if q = 2 then
        ...
        if IsBound(Cold.coveringRadius) then
            C.coveringRadius := Cold.coveringRadius + nrcolumns;
        fi;
    else
...
UnrestrictedDirectSumCode := function (C1, C2) 
    ...
    if IsBound(C1.coveringRadius) and IsBound(C2.coveringRadius) then
        C.coveringRadius := CoveringRadius(C1) + CoveringRadius(C2);
    fi;
