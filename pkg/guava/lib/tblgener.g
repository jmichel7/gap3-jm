...
##
CreateBoundsTable := function( arg )
    local RulesList, LBT, UBT, FillPoints, NumberUnchanged, RuleNumber, file,
          WriteToFile, InitialTable, q, Sz, info, help, temp;
    ...
    info := (Length(arg) = 3) and arg[3];

    InitialTable := function(to, lb)
        local n, k, d, i, j, sum, BT;
        BT := List([1..to], i-> [[i]]);      #RepititionCodes
        for k in [2 .. to] do
    ...
    FillPoints := function( BT, lb )
        ...
        if lb then
            initialfile := Concatenation(LOADED_PACKAGES.guava,"tbl/codes",
                                   String(q),".g");
        else
            initialfile := Concatenation(LOADED_PACKAGES.guava,"tbl/upperbd",
                                   String(q),".g");
        fi;
        if not READ(initialfile) then
...
#F              begin of body
    ...
    file := Concatenation(LOADED_PACKAGES.guava,"tbl/bdtable",String(q),".g");
