...
void standard_presentation ( Logical *identity_map, int standard_output, int ***auts, 
...
int**start_pga_run(Logical*identity_map,int***auts,struct pga_vars*pga,
  struct pcp_vars*pcp)
{
...
int**finish_pga_run(Logical*identity_map,FILE_TYPE cover_tmp_file,
  FILE_TYPE group_file,int***auts,struct pga_vars*pga,struct pcp_vars*pcp)
{
   int **perms;
   ...
   if (!soluble_group) {
	 ...
	 process_fork = TRUE;
      ...
      start_GAP_file (&LINK_input, auts, pga);
#endif
#endif
#endif
#endif
...
void list_subgroup (rep, pga, pcp)
int rep;
struct pga_vars *pga;
struct pcp_vars *pcp;
{
...
int**find_stabiliser(Logical*identity_map,int non_standard,int***auts,int**perms,
  int*a,int*b,char*c,int*orbit_length,struct pga_vars *pga,struct pcp_vars *pcp)
{
   FILE_TYPE Status;
...
void print_aut_description ( int ***central, int ***stabiliser, struct pga_vars *pga, 
  struct pcp_vars *pcp)
{
   FILE_TYPE NextClass;
   int i, j, k;
