...
extern short  perm[],sv[],cp[],fpt[],orb[],intpow[],
       ...
       intno[],ngno[],power[],wt[],co[],rel[],d1[],d2[],
       igno[],pinv[],rel[],pwt[],*sv2ptr[],
       crep[],crepinv[],dcrep[],dcrepinv[],tp[],
       mp,mb,mpt,mexp;
extern int psp,svsp;
char  norm;
...
int expint(i,j,nwt) short i,j,nwt;
...
{ short l,m,n,ino,pt,p,*g,*ig;  char diff;
  ...
  { firstgen(tp,&hgen,&coeff);
    if (hgen==0) break;
    g=pptr[intno[hgen]]; ig=g+npt1;
    if (g==0) {printf("Subint error.\n"); return(-1); }
    ino=g[npt1];
    if ((wt[ino]<nwt) || (wt[ino]==nwt && d1[ino]==0))
    { wt[ino]=nwt; intpow[ino]=pinv[coeff]; diff=0;
      for (n=1;n<=npt;n++) if (g[n]!=tp[n]) {diff=1; break;}
      if (diff)
      { for (n=1;n<=npt;n++) {pt=tp[n]; g[n]=pt; ig[pt]=n; }
        for (n=1;n<=intexp;n++) {d1[n]=0; d2[n]=0;}
        return(ino);
      }
      else { d1[ino]=i; d2[ino]=j;}
    }
    l++; rel[l]=1+intexp-ino; l++; p=(coeff*intpow[ino])%prime; rel[l]=p;
    for (n=1;n<=npt;n++)
    { pt=itp[n]; for (m=1;m<=p;m++) pt=g[pt]; tp[pt]=n; }
    invert(tp,itp);
  }
  if (nwt) return(0);
  m=1+intexp-rel[l-1];
  if (d1[m]==i && d2[m]==j) rel[0]=rel[l-1]; else rel[0]=0;
  rel[1]=l-1;
