...
{ short i,j;
  ...
  for (i=1;i<=npt;i++)
  { scanf("%hd",ptr+i); j=ptr[i];
    if (j<=0 || j>npt || orb[j]) return(2);
    orb[j]=1;
  }
  return(0);
}
...
{ short orct,lo,u,v,w,x,y,z;
  ...
  { orct++; orno[u]=orct; lo=1; orb[1]=u;
    for (x=1;x<=lo;x++)
    { z=orb[x];
      for (y=1;y<= *cp;y++)
      { w=cp[y]; v=pptr[w][z];
        if (orno[v]==0) {lo++; orb[lo]=v; orno[v]=orct; }
      }
    }
    printf("Orbit number %3d,  length %3d:\n",orct,lo);
    for (x=1;x<=lo;x++) printf("%4d",orb[x]); printf("\n");
  }
  return(0);
}
