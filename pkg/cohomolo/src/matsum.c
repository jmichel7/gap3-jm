...
{ short prime,dim1,dim2,nmats,i,j,k,n,err;
  ...
  err=0;
  strcpy(inf1,argv[1]); strcat(inf1,"."); strcpy(inf2,inf1); strcpy(outf,inf1);
  strcat(inf1,argv[2]); strcat(inf2,argv[3]); strcat(outf,argv[4]);
  ...
  for (i=1;i<=nmats;i++)
  { for (j=1;j<=dim1;j++)
    { for (k=1;k<=dim1;k++) { fscanf(ip1,"%hd",&n); fprintf(op,"%4d",n);}
      for (k=1;k<=dim2;k++) fprintf(op,"%4d",0);
      fprintf(op,"\n"); 
    }
    for (j=1;j<=dim2;j++)
    { for (k=1;k<=dim1;k++) fprintf(op,"%4d",0);
      for (k=1;k<=dim2;k++) { fscanf(ip2,"%hd",&n); fprintf(op,"%4d",n);}
      fprintf(op,"\n"); 
    }
  }
