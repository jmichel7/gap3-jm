...
*/
int  mng=MNG-1,mexp=MEXP-1,wksp=WKSP,
       prime,exp,nng,class,*rpf,*rpb,*eexpnt,*enexpnt,**pcb,mnng,mord,intexp,
       rel[RSP],expnt[MEXP],nexpnt[MNG],cord[MNG],wt[MEXP],
       d1[MEXP+MNG],d2[MEXP+MNG],*pcptr[PTRSP],**powptr[MEXP],**comptr[MEXP],
       *sspc[MEXP],*sspf[MEXP],sgen[MEXP],sex[MEXP],spgen[MEXP],spex[MEXP],
...
*/
{ int i,j,k,l,m,x,y,no,*orpf,*orpb,**pcp;  char tails;
 /* ip=fopen(inf1,"r");
  while(EOF!=(i=fgetc(ip))){putchar(i);}
  fclose(ip);*/
  if ((ip=fopen(inf1,"r"))==0)
  { fprintf(stderr,"Cannot open %s.\n",inf1); return(-1); }
  fscanf(ip,"%d%d%d%d%d%d",&prime,&exp,&nng,&no,&class,&m);
  ...
  for (i=1;i<=exp;i++) fscanf(ip,"%d",wt+i);
  for (i=1;i<=exp+nng;i++) fscanf(ip,"%d",d1+i);
  for (i=1;i<=exp+nng;i++) fscanf(ip,"%d",d2+i);
  for (i=1;i<=nng;i++) fscanf(ip,"%d",cord+i);
  ...
  { comptr[i]=pcp-2;
    ...
    { fscanf(ip,"%d%d",&k,&l);
      ...
      { *(pcp++)=rpf+1; *(rpf++)=k; *rpf=l; orpf=rpf; rpf +=(l+1);
        while ((++orpf)<rpf) fscanf(ip,"%d",orpf);
      }
      if (tails)
      { fscanf(ip,"%d%d",&k,&l);
        if (l==0) *(pcp++)=0;
        ...
        { orpb=rpb-nng; *(pcp++)=orpb; *orpb=k;
          ...
          l/=2; for (k=1;k<=l;k++) {fscanf(ip,"%d%d",&x,&y); rpb[x]=y;}
          rpb--;
        }
      }
      else *(pcp++)=0;
  ...
  { powptr[i]=pcp;
    ...
    { fscanf(ip,"%d%d",&k,&l);
      ...
      { orpb=rpb-nng; *(pcp++)=orpb; *orpb=k;
        while (rpb>orpb) *(rpb--)=0;
        l/=2;
        for (k=1;k<=l;k++) {fscanf(ip,"%d%d",&x,&y); rpb[x]=y;}
        rpb--;
      }
    }
    else *(pcp++)=0;
...
void zero(p1,p2) int *p1,*p2;
{ while ((++p1)<=p2) *p1=0; }

int intgen(i,j) int i,j;
...
int collect(int *spc,int *spf,int sgn)
...
*/
{ int gen,ex,pgen,pex,pugen,stkp,exsgn,*e,*p1,*p2,*p3,*p4,**dp;
...
/* The difference between the tails of the elements (g1g2)g3 and g1(g2g3) is
   ...
   g2=g3.
*/
{ char eq12,eq23,prnt,triv; int i,l,e,*p;
...
*/
{ int i,j,k,len,hp,ct,fac,x,y,a,b,c,hpi,gno,*p,**dp,term[30];
...
/* The data structures for this program and for nqrun are similar.
   ...
   and powptr. More precisely, they are pointed to by pointers stored in pcptr,
   and it is these pointers that are pointed to by comptr and powptr.
...
*/
int nqmprog()
{ int i,j,k,l,m,d,*gi,*gj,*ti,*tj,cl,def,*ps,*pf,**dp,*nrpb,*p,*orpf,*orpb,
...
*/
  if (nng==0)
  { mnng=0; for (i=1;i<=exp;i++) if (wt[i]>1) mnng++;
    for (i=1;i<=exp;i++) for (j=1;j<i;j++)
    { gi= *(comptr[i]+2*j);
...
printf("mnng=%d\n",mnng);
    j=exp/2; mord=1;
    for (i=1;i<=j;i++) mord*=prime;
/* Now we start introducing the new generators */
    for (cl=class+1;cl>=2;cl--)
    { for (i=1;i<exp;i++) for (j=i+1;j<=exp;j++)
      if (wt[i]+wt[j]==cl && (wt[i]==1 || wt[j]==1))
      if (intgen(j,i)== -1) return(-1);
      for (i=3;i<=exp;i++) if (wt[i]==cl) if (intgen(i,i)== -1) return(-1);
      for (i=3;i<exp;i++) if (wt[i]>1) for (j=i+1;j<=exp;j++)
      if (wt[j]>1 && wt[i]+wt[j]==cl)
      { k=d1[i]; l=d2[i];
        if (assoc(j,k,l)) if (subrel(j,i)== -1) return(-1);
      }
      for (j=1;j<=exp;j++) for (i=1;i<=j;i++) if (wt[i]+wt[j]==cl)
      { if (assoc(j,i,i)) if ((l=prnrel(0))== -1) goto nextcl;
         if (j!=i && assoc(j,j,i)) if ((l=prnrel(0))== -1) goto nextcl;
      }
      for (k=3;k<=exp;k++) for (j=2;j<k;j++) for (i=1;i<j;i++)
      if (wt[i]+wt[j]+wt[k]==cl && assoc(k,j,i))
      if ((l=prnrel(0))== -1) goto nextcl;
nextcl:;
    }
    if (nng==0)
    { if (gap)
      ...
      else fprintf(stderr,"Multiplier is trivial.\n");
      return(-1);
    }
...
/* First we read in the genrators and their images under the action.
   These images will later have their own tail in the new generators, which
   will be stored in the arrays tlintg[i].
*/
      rpf=orpf; rpb=orpb;
      if (norm==0) for (i=1;i<=intexp;i++)
      { fscanf(ip,"%d",rpf); intg[i]=rpf; l= *rpf;
        for (j=1;j<=l;j++) {rpf++; fscanf(ip,"%d",rpf); }
        rpf++;
      }
      for (i=1;i<=intexp;i++)
      { fscanf(ip,"%d",rpf); imintg[i]=rpf; tlintg[i]=0; l= *rpf;
        for (j=1;j<=l;j++) {rpf++; fscanf(ip,"%d",rpf); }
        rpf++;
      }
      if (rpb-rpf<wksp)
...
/* Now we compute the action */
      if (norm)
      { wf=rpf;
        for (l=3;l<=exp;l++) if ((i=d1[l])!=0)
        { j=d2[l]; p= *(comptr[i]+2*j); ps=p+1; pf=p+*p -1;
          zero(nexpnt,enexpnt); p=pf-2; wc=wf-2;
          while (p>=ps) { enter(imintg[*p],-*(p+1),tlintg[*p]); p-=2; }
          gi=imintg[i]; gj=imintg[j]; ti=tlintg[i]; tj=tlintg[j];
          enter(gi,-1,ti); enter(gj,-1,tj); enter(gi,1,ti); enter(gj,1,tj);
          zero(expnt,eexpnt); collect(wc,wf,1);
          nt=0;
          for (k=1;k<=nng;k++) if (nexpnt[k]!=0) { nt=1; break; }
          if (nt)
          { nrpb=rpb-nng; zero(nrpb,rpb); tlintg[l]=nrpb;
            for (k=1;k<=nng;k++) nrpb[k]=nexpnt[k];
            rpb=nrpb;
          }
        }
      }
      for (i=1;i<=intexp;i++) for (j=1;j<=i;j++)
      { if (ims==0 && i==j) continue;
        if (norm)
        { if (ims && i==j) dp=powptr[i]; else dp=comptr[i]+2*j;
          p= *dp;
          ...
          covrel= *(dp+1);
          if (ims) if (covrel==0 || *covrel==0) continue;
        }
        else
        { if (i==intexp) {def=0; *rpf=0; }
          else
          { fscanf(ip,"%d%d",&def,rpf);
            for (k=1;k<= *rpf;k++) fscanf(ip,"%d",rpf+k);
          }
          ps=rpf+1; wf=ps+*rpf; pf=wf-2;
        }
        if (def)
        { zero(nexpnt,enexpnt); p=pf-2; wc=wf-2;
          while (p>=ps) { enter(imintg[*p],-*(p+1),tlintg[*p]); p-=2; }
          ...
          zero(expnt,eexpnt); collect(wc,wf,1);
          p=pf-2; wc=wf-2;
          while (p>=ps) {enter(intg[*p],-*(p+1),0); p-=2;}
          gi=intg[i]; gj=intg[j];
          enter(gi,-1,0); enter(gj,-1,0); enter(gi,1,0); enter(gj,1,0);
          zero(expnt,eexpnt); collect(wc,wf,-1);
          nt=0;
          for (k=1;k<=nng;k++) if (nexpnt[k]!=0) { nt=1; break; }
          if (nt)
          { nrpb=rpb-nng; zero(nrpb,rpb); tlintg[def]=nrpb;
            for (k=1;k<=nng;k++) nrpb[k]=nexpnt[k];
            rpb=nrpb;
          }
        }
        ...
        { zero(nexpnt,enexpnt); zero(expnt,eexpnt);
          ...
          p=pf; wc=wf-2;
          while (p>=ps) { enter(imintg[*p],-*(p+1),tlintg[*p]); p-=2;}
          if (i==j) enter(imintg[i],prime,tlintg[i]);
          else
          { gi=imintg[i]; gj=imintg[j]; ti=tlintg[i]; tj=tlintg[j];
            enter(gi,-1,ti); enter(gj,-1,tj); enter(gi,1,ti); enter(gj,1,tj);
          }
          zero(expnt,eexpnt); collect(wc,wf,1);
          if (ims)
          { for (k=1;k<=nng;k++) printf("%2d",covrel[k]); printf("    ->    ");
            for (k=1;k<=nng;k++)
            { m=nexpnt[k]; if (m<0) { l=cord[k]; m+=l; }
              printf("%2d",m);
            }
            printf("\n");
          }
          ...
          {  if (norm && covrel)
            for (k=1;k<=nng;k++)
            { l=covrel[k]; nexpnt[k]-=l; l=cord[k]; nexpnt[k]%=l;}
  ...
  { op=fopen(outfm,"w");
    ...
    for (i=1;i<=nng;i++)
    { fprintf(op,"%d",cord[i]); if (i<nng) putc(',',op);}
    fprintf(op,"];\n");
    fclose(op);
    printf("Orders of the cyclic factors of the multiplier are now:\n");
    for (i=1;i<=nng;i++) printf("%3d  ",cord[i]); printf("\n");
  }
  else
  { fprintf(stderr,"Orders of the cyclic factors of the multiplier are now:\n");
    for (i=1;i<=nng;i++) fprintf(stderr,"%3d  ",cord[i]); fprintf(stderr,"\n");
  }
  if (crel)
  { strcpy(inf1,inf0); strcat(inf1,"psgwds");
    if ((ip=fopen(inf1,"r"))==0)
    { printf("Cannot open %s.\n",inf1); return(-1); }
    pgen=pcb; fscanf(ip,"%d",&np);
    if (pcb+np-pcptr>=ptrsp)
    { fprintf(stderr,"Out of ptr space. Increase PTRSP.\n"); return(-1); }
    for (i=1;i<=np;i++)
    { pgen[i]=rpf; fscanf(ip,"%d",rpf); p=rpf; rpf+=(1+ *p);
      while (++p<rpf) fscanf(ip,"%d",p);
    }
    fclose(ip);
    strcpy(inf1,inf0); strcat(inf1,"psg.rel");
    if ((ip=fopen(inf1,"r"))==0)
    { fprintf(stderr,"Cannot open %s.\n",inf1); return(-1); }
    strcpy(outf,inf0); strcat(outf,"psg.er"); op=fopen(outf,"w");
    fscanf(ip,"%d",&nb); rno=rpf-1; rpf+=nb;
    if (rpb-rpf<wksp)
    { fprintf(stderr,"Out of space. Increase RSP.\n"); return(-1); }
    for (i=1;i<=nb;i++) fscanf(ip,"%d",rno+i);
    fprintf(op,"%4d%4d\n",nb,nng);
    for (i=1;i<=nb;i++) fprintf(op,"%4d",rno[i]); fprintf(op,"\n");
    for (i=1;i<=nng;i++) fprintf(op,"%4d",cord[i]); fprintf(op,"\n");
    wf=rpf;
    for (i=1;i<=rno[1];i++)
    { fscanf(ip,"%d",&l); covrel=rpb-l; p=covrel;
      while (++p<=rpb) fscanf(ip,"%d",p);
...
{ int arg,i,j,k,*pb,*pc,*pe;  char err,c,d;
  ims=0; act=0; crel=0; gap=0; err=0; arg=1;
  if (argc<=arg) {err=1; goto error;}
  while (argv[arg][0]=='-')
  { if (argv[arg][1]=='a') act=1; else if (argv[arg][1]=='c') crel=1;
    else if (argv[arg][1]=='i') ims=1; else if (argv[arg][1]=='g') gap=1;
    else {err=1; goto error; }
    arg++; if (argc<=arg) {err=1; goto error; }
  }
  strcpy(inf0,argv[arg]); strcat(inf0,".");
  strcpy(outf,inf0); strcpy(outfm,inf0); strcat(outfm,"mult");
  strcpy(inf1,inf0);
  if (act)
  { strcpy(inf2,inf1);
    arg++; if (argc<=arg) strcat(inf2,"sc"); else strcat(inf2,argv[arg]);
  }
  arg++;
  if (argc<=arg) if (act || crel) strcat(inf1,"cov"); else strcat(inf1,"pcp");
  else strcat(inf1,argv[arg]);
  arg++; if (argc<=arg) strcat(outf,"cov"); else strcat(outf,argv[arg]);
  if (nqmprog()== -1) exit(1);
  error:
  if (err)
