...
{ short i,j,l,m,n,*p,**dp,maxv,maxm,ord;
  ...
  { f=1; seeknln();
reenter:
    printf("Input filename:  "); scanf("%s",flnm);
    if ((ip=fopen(flnm,"r"))==0)
    { fprintf(stderr,"Cannot open %s.\n",flnm);
      seeknln(); goto reenter;
    }
    fscanf(ip,"%hd%hd%hd",&prime,&dim,&n);
    if (prime>mpr) {fprintf(stderr,"prime too big.\n"); exit(1);}
  }
  else
  {f=0;printf("Input modulus,dim:    "); scanf("%hd%hd",&prime,&dim);}
  if (prime>mpr) {fprintf(stderr,"modulus too big.\n"); exit(1);}
  if (dim>mdim) {fprintf(stderr,"dim too big.\n"); exit(1);}
  seeknln(); setpinv();
/* Set matrix and vector pointers as in mcd.c  */
  quot=msp/dim; if (quot>=mv) quot=mv-1; maxv=quot;
  for (i=1;i<=maxv;i++) vec[i]= mspace-1+(i-1)*dim;
  maxm= (maxv-10)/dim-1; if (maxm>=mm) maxm=mm-1;
  for (i=0;i<=maxm;i++) mat[i]=vec+10+i*dim;
  dp=mat[0]; p= *dp;
  for (i=1;i<=dim*dim;i++) *(++p)=0;
  for (i=1;i<=dim;i++) dp[i][i]=1;
  spm=mat[maxm]; spv=spm[1];
  printf("10 free vectors and %d matrices can be defined.\n",maxm-1);
  printf("Mat 0 is the identity.\n");
  if (f) { for (i=1;i<=n;i++) readmat(mat[i]);  fclose(ip); }
  while(1)
  { printf("Choose option. Print 'l' for list.\n");
    ...
    { scanf("%hd",&n); seeknln();
      ...
      for (i=1;i<=dim;i++)
      { scanf("%hd",&m); m%=prime; if (m<0) m+=prime; vec[n][i]=m; }
      seeknln();
    }
    ...
    { scanf("%hd",&n); seeknln();
      ...
      { scanf("%hd",&m); m%=prime; if (m<0) m+=prime; mat[n][i][j]=m; }
      seeknln();
    }
    ...
    { scanf("%hd",&n); seeknln();
      if (n<=0 || n>=10)
      { fprintf(stderr,"Invalid vector number %d\n",n); continue;}
      for (i=1;i<=dim;i++) printf("%3d",vec[n][i]); printf("\n");
    }
    ...
    { scanf("%hd",&n); seeknln();
      ...
      { fprintf(stderr,"Invalid matrix number %d\n",n); continue;}
      for (i=1;i<=dim;i++)
      { for (j=1;j<=dim;j++) printf("%3d",mat[n][i][j]); printf("\n"); }
    }
    else if (strcmp(opt,"tr")==0)
    { scanf("%hd%hd",&m,&n); seeknln();
      if (n<=0 || n>=maxm)
      { fprintf(stderr,"Invalid matrix number %d\n",n); continue;}
      if (m<0 || m>=maxm)
      { fprintf(stderr,"Invalid matrix number %d\n",m); continue;}
    ...
    { scanf("%hd%hd%hd",&l,&m,&n); seeknln();
      ...
      if (n<0 || n>=maxm)
      { fprintf(stderr,"Invalid matrix number %d\n",n); continue;}
      if (strcmp(opt,"im")==0) im(vec[l],vec[m],mat[n]);
      else comm(vec[l],vec[m],mat[n]);
    }
    ...
    { scanf("%hd%hd",&n,cp);
      ...
      for (i=1;i<=dim;i++) for (j=1;j<=dim;j++)
      { m=0; for (l=1;l<= *cp;l++)
        if (cp[l]>0) m+=mat[cp[l]][i][j]; else m-=mat[-cp[l]][i][j];
        m%=prime; if (m<0) m+=prime; mat[n][i][j]=m;
      }
    }
    else if (strcmp(opt,"ch")==0)
    { scanf("%hd%hd%hd%hd",&n,&i,&j,&l); seeknln();
      if (n<=0 || n>=maxm)
      { fprintf(stderr,"Invalid matrix number %d\n",n); continue;}
    ...
    { scanf("%hd%hd",&m,&n); seeknln();
      ...
      f=1;
      for (i=1;i<=dim;i++)
      { for (j=1;j<=dim;j++) if (mat[m][i][j]!=mat[n][i][j]) { f=0; break; }
        if (f==0) break;
      }
      if (f) printf("Matrices are equal.\n");
    ...
    { scanf("%hd%hd",&m,&n); seeknln();
      ...
      if (m<0 || m>=maxm)
      { fprintf(stderr,"Invalid matrix number %d\n",m); continue;}
      *cp=1; cp[1]=m; prod(cp,mat[n]); prod (cp,spm); ord=1;
      while(1)
      { f=1;
        for (i=1;i<=dim;i++)
        { for (j=1;j<=dim;j++) if (mat[0][i][j]!=spm[i][j]) { f=0; break; }
          if (f==0) break;
        }
        if (f) { printf("Order=%d\n",ord); break;}
        *cp=1; cp[1]=maxm;  prod(cp,mat[n]);
        ord++; *cp=2; cp[1]=m; cp[2]=n; prod(cp,spm);
      }
    }
    else if (strcmp(opt,"op")==0)
    { scanf("%s",flnm); op=fopen(flnm,"w");
      ...
      seeknln();
      if (fault) { fclose(op); unlink(flnm); continue;}
      fclose(op);
    }
    else if (strcmp(opt,"q")==0) {seeknln(); break;}
    else { seeknln(); printf("Invalid option.\n");}
  }
  exit(0);
}
