...
# define  MDIM       100
# define    MB       80
char slg,con,check,inf1[80],inf2[80],inf3[80],inf4[80],inf5[80],inf6[80],
     outf1[80];
...
short mm=MM,mv=MV,mp=MP,mpt=NPT,
      mpr=MPR,mdim=MDIM,mb=MB,
...
{ short arg;  char c,err;
  ...
  c=argv[arg][0];
  while (c=='-')
  { c=argv[arg][1]; if (c=='o') slg=0;
    else if (c=='t') check=1;
    else {err=1; goto error; }
    arg++; if (argc<=arg) {err=1; goto error; }
    c=argv[arg][0];
  }
  strcpy(inf1,argv[arg]);strcat(inf1,".");strcpy(inf2,inf1);strcpy(inf3,inf1);
  strcpy(inf4,inf1); strcpy(inf5,inf1); strcpy(inf6,inf1);
  strcat(inf1,"inmat"); strcat(inf2,"words");
  if (slg) strcat(inf3,"sg"); else strcat(inf3,"outperm");
  if (check) strcat(inf6,"sg.rel");
  if (mcprog()== -1) exit(1);
  while(1)
...
*/
  { arg++; if (argc<=arg) break;
    if (strcmp(argv[arg],"pg")==0)
    { con=1; strcpy(inf6,inf5); strcat(inf6,"cbmats");}
    else if (argv[arg][0]=='-') con=2;
    else con=0;
    strcpy(inf4,inf5);
    if (con==2) strcat(inf4,argv[arg]+1); else strcat(inf4,argv[arg]);
    strcpy(outf1,inf4); strcat(outf1,"mat");
    if (conprog(con)== -1) exit(1);
  }
error:
  if (err)
