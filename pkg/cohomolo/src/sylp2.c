...
extern short  mp,mexp,mb,mnpt,par1,par2,par3,par4,prime,
       ...
       gbase[],start[],pno[],genorb[],*pptr[],*svgptr[],*svhptr[],*expptr[],
       obase[],nbase[],ntorno[],lorbn[],reg[],ipno[],
       tsv1[],tsv2[],tsv3[],orep[],*intorb[];
extern int psp,sp,svsp;

short npt,im,cb,adno,*tp,*itp,nb,bno,lexp,np2,
...
/* This program and normrun involve backtrack searches. These can be speeded up
   ...
   stored in this way, in array space, using perm pointer expptr, and expcp
   corresponds to cp.
*/
...
*/
{ short i,j,k,l; i=orno[cb]; j=orno[im]; k=orbp[i];
  if (k==0)
  { if (deft[j]==0) if (lporb[i]==lporb[j])
    { orbp[i]=j; deft[j]=adno;
      if (i==opno)
      { l=j;
        while (l!=bnoorno && l!=0) { opct++; opno=l; l=orbp[l];}
        if (l!=0 && opct%prime!=0) { ok=0; bt=1;}
      }
    }
    else ok=0; else ok=0;
  }
...
{ short i,k,l;
  ...
  { l=rand()%lorbg[i];
    k=gorb[i][l];
    if (i==bno && svhptr[bno][k]!=0) { i=ntbpt[i]; continue;}
    if (k!=gbase[i])
    { (*expcp)++;
      if (i<=lexp) { expcp[*expcp]=i; exprep(k,i,svgptr[i]);}
      else expcp[*expcp]= (k>gbase[i]) ? start[i]+l : start[i]+l+1;
    }
  }
...
{ short ord,i,j,k,l;
  ord=1;
  for (i=1;i<=npt;i++) if (ip[i]==0)
  { j=i; l=1; k=p[j]; ip[j]=j;
    while (k!=i) { l++; j=k; k=p[j]; ip[k]=j;}
    ord=lcm(ord,l);
  }
  printf("ord=%d\n",ord);
  return(ord);
