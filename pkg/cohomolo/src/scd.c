...
char mult,subgp,sgc,sgstr[2],
     inf0[80],inf1[80],inf2[80],inf3[80],inf4[80],outf[80],outft[80];
...
short  perm[PSP],sv[SVSP],cp[2*NPT],fpt[MEXP],orb[NPT+1],intpow[MP],
       ...
       intno[MEXP],ngno[MEXP],power[MEXP],wt[MEXP],co[MEXP],rel[2*MEXP],
       igno[2*MEXP],pinv[NPT/2],*sv2ptr[MB],d1[MEXP],d2[MEXP],pwt[MEXP],
       crep[NPT+1],crepinv[NPT+1],dcrep[NPT+1],dcrepinv[NPT+1],tp[2*(NPT+1)],
...
{ short arg;  char c,err,ondef;
  ...
  { c=argv[arg][1];
    if (c=='s')
    { sgc=argv[arg][2];
      if (sgc=='\0') { subgp=1; sgc='1'; ondef=0;}
      else { subgp=sgc-'0'; sgstr[1]='\0'; ondef=1;}
      if (subgp<=0 || subgp>9) {err=1; goto error; }
    }
    else if (c=='m') mult=1;
