##############################################################################
##
#A  coho.g                     February 1995                        Derek Holt
##
...
#H
##
##  This file contains the interface to my cohomology programs.
##
if not IsBound(InfoCohomology)  then InfoCohomology := Ignore;  fi;
...
COHOMOLO := rec(
                 ...
                 PermRels   := [],
                 RM_F       := "rm -f ",
                 CALL       := Concatenation( "cd ", PKGNAME[1],
                                              "cohomolo/bin/ ; ./" )
                 #This is the directory of the external executables
                );

COHOMOLO.Exec:=function(s)
   InfoCohomology( "#I  Cohomology package: Executing\n#     ",s,"\n" );
   Exec(s);
   InfoCohomology( "#I  External program complete.\n" );
end;
...
COHOMOLO.WriteMatrices := function ( mats, filename)
   ...
   for i in [1..nm] do
     ...
     for j in [1..dim] do
        ...
        for k in [1..dim] do
          entry := row[k];
          if entry = f.zero then n := 0; else n := table[LogFFE(entry,w)+1]; fi;
...
CHR := function ( arg )
   ...
   if mats<>0 then
     if not IsList(mats) or Length(mats)<>ng then
       ...
       for i in [1..ng] do
         mat := mats[i];
         if not IsMat(mat) or Field(Flat(mat))<>GF(p) then
            error := true;
         else
            d := DimensionsMat(mat);
...
##
IsCHR := function(x)
   return IsRec(x) and IsBound(x.isCHR) and x.isCHR=true;
end;
...
SchurMultiplier := function( chr )
   if not IsCHR(chr) then
     Error("First argument of Cohomology must be a cohomology record");
   fi;
...
CoveringGroup := function( chr )
   ...
   if not IsBound(chr.multrelvals) then
      Cohomology(chr,true,true,false, TmpName() );
   fi;
...
FirstCohomologyDimension := function( chr )
   ...
   if not IsBound(chr.codim1) then
      Cohomology(chr,false,false,true, TmpName() );
   fi;
   return chr.codim1;
end;
...
SecondCohomologyDimension := function( chr )
   ...
   if not IsBound(chr.codim2) then
      Cohomology(chr,false,false,false, TmpName() );
   fi;
   return chr.codim2;
end;
...
NonsplitExtension := function( arg )
   ...
   if not IsBound(chr.modrelvals) then
      Cohomology(chr,false,true,false, TmpName() );
   fi;
...
#############################################################################
##
#F  Cohomology( <chr>, <mult>, <pres>, <first>, <filename> )
...
##
Cohomology := function( chr, mult, pres, first, filename )
   ...
   if not mult and mats=0 then
     Error("Cohomology: if mult is false, matrices field must be defined");
   fi;
   if first and mats=0 then
     Error("Cohomology: if first is true, matrices field must be defined");
   fi;
   if first and (mult or pres) then
     Error("Cohomology: if first is true, mult and pres must be false");
   fi;
   if pres and not IsRec(F) then
     Error("Cohomology: if pres is true, fpgp field must be defined");
   fi;
   ...
   if Set(FactorsInt(Size(P))) <> [p] or p in FactorsInt(Index(G,P)) then
      Error("Cohomology: sylow field of <chr> is not a Sylow-subgroup!");
   fi;
...
"The Sylow p-subgroup of the group is trivial - so all cohomology is trivial.\n"
     ...
     chr.multiplier := [];
     if IsRec(F) then chr.multrelvals := []; fi;
     if IsRec(F) and mats<>0 then chr.modrelvals:=[]; fi;
     return;
   fi;
...
"The Sylow p-subgroup of the group is cyclic - so the multiplier is trivial.\n"
   ...
   if not ok then
      Error(
    "Cohomology: <chr>.chain must be a strictly decreasing chain from G to P.");
   fi;
   ...
   callstring := Concatenation( COHOMOLO.CALL, "cohomology.gap ");
   optstring := "-";
   ...
   InfoCohomology( "#I  Removing temporary files.\n" );
   Exec( Concatenation( COHOMOLO.RM_F, filename, ".*" ) );
end;
...
##  is false.
##  In the first case, Cohomology must already have been called on <chr> with
...
##  in which case the split extension is returned, or Cohomology must already
...
CalcExtPres := function( arg )
    local chr, mult, vec, E, EG, p, F, ng, codim, mp, dim, rels, rel, relval, w,
          mats, row, modrv, multrv, f, table, rt, i, j, k;
   ...
   if mult then
      if not IsBound(chr.multiplier) or not IsBound(chr.multrelvals) then
         Error("CalcExtPres: Cohomology must be run before CalcExtPres");
      fi;
   ...
   for i in [1..Length(rels)] do
      rel := rels[i];
      w := MappedWord(rel,F.generators,Sublist(EG,[1..ng]));
      if mult then
         if mp=[] then relval := [];
...
FindSubSeq := function ( chr, n )
      ...
      while not foundsub and orbsize <= 16 do
         ...
         while not foundsub and orbno <= norbs do
            if not orbdone[orbno] and Length(orb[orbno]) = orbsize then
               if orb[orbno] = Orbit(H,orb[orbno][1]) then
                  orbstab := H;
               else
   ...
   InfoCohomology("#Indices in the subgroup chain are:  ");
   badind := false;
   for i in [1..Length(seq)-1] do
     ind := Index(seq[i],seq[i+1]);
     if ind>50000 then badind := true; fi;
     InfoCohomology(Index(seq[i],seq[i+1])," ");
   od;
   InfoCohomology("\n");
...
CalcPres := function( chr )
   ...
   #Now work out the command for the external program.
   callstring := Concatenation( COHOMOLO.CALL, "calcpres.gap " );
   optstring := "-";
   if chr.verbose then Add( optstring, 'v' ); fi;
   if 1 < Length( optstring ) then
      Append( callstring, optstring );
