...
checked  case-by  case  is  that,  for  a unipotent character $\gamma$ with
polynomial  generic  degree  $\deg\gamma(q)$  of  the spets attached to <W>
(this  spets is a  finite reductive group,  for <W> a  Weyl group, in which
case  $z=-1$  if  $-1$  is  in  <W>), $\deg\gamma(zq)$ is equal to $\pm\deg
En(\gamma)(q)$  for  another  unipotent  character $En(\gamma)$, the Ennola
transform of $\gamma$. The function returns the permutation-with-signs done
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\Section{PermutationOnUnipotents}
\index{PermutationOnUnipotents}

'PermutationOnUnipotents(<W>,<aut>)'

<W>  is  a  reflection  group  or  reflection  coset  representing a finite
reductive group $\bG^F$, and <aut> is an automorphism of $\bG^F$ (for $W$ a
permutation  group, this can be  given as a permutation  of the roots). The
function  returns the  permutation of  the unipotent  characters of $\bG^F$
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\Section{The $d$-Harish-Chandra series}
$d$-Harish-Chandra   series  describe  unipotent  $l$-blocks  of  a  finite
reductive  group $\bG(\BF_q)$ for $l\|\Phi_d(q)$ (at  least, when $l$ is not
too  small which means mostly not a bad prime for $\bG$). Some of the facts
stated  below are  still partly  conjectural, we  do not try to distinguish
precisely what has been established and what is still conjectural.

If  $(\bL,\lambda)$ is  a $d$-cuspidal  pair then  the constituents  of the
Lusztig   induced  $R_\bL^\bG(\lambda)$  are  called  a  $d$-Harish-Chandra
series;  they form  the unipotent  part of  an $l$-block  of $\bG^F$. It is
conjectured  (and  proven  in  some  cases)  that  the $\bG^F$-endomorphism
algebra  of the $l$-adic cohomology of  the variety $\bX$ which defines the
Lusztig  induction is  a $d$-cyclotomic  Hecke algebra $H_\bG(\bL,\lambda)$
for  the  group  $W_\bG(\bL,\lambda)\:=N_\bG(\bL,\lambda)/\bL$,  which is a
complex  reflection group --- here $d$-cyclotomic means that the parameters
of  $H_\bG(\bL,\lambda)$ are monomials in $q$ and that $H_\bG(\bL,\lambda)$
specializes to the algebra of $W_\bG(\bL,\lambda)$ for $q\mapsto$|E(d)|.

It  follows that the decomposition of the  Lusztig induction is of the form
$$R_\bL^\bG(\lambda)=\sum_{\phi\in\hbox{Irr}(W_\bG(\bL,\lambda))}(-1)^{n_\phi}
\phi(1)\gamma_\phi,$$  where  $\gamma_\phi$  is  a  unipotent  character of
$\bG^F$ attached to $\phi$ and where $n_\phi$ is the degree
$H^{n_\phi}_c(\bX)$ where $\gamma_\phi$ occurss; and further for any $\phi$
we  have $R_\bL^\bG(\lambda)(1)=  (-1)^{n_\phi} \gamma_\phi(1)S_\phi$ where
$S_\phi$  is  the  Schur  element  of the character of $H_\bG(\bL,\lambda)$
which  deforms  to  $\phi$.  The  function  |Series|  allows  to  explore a
...
Above we explore the 3-series corresponding to $R_\bT^\bG(\hbox{Id})$ where
$\bG$  is the triality group and $\bT$  is the torus of type $(q^2+q+1)^2$.
The  group $W_\bG(\bT)$ is the complex reflection group $G_4$. The displays
shows   in  the  column  'Name'  the   name  of  the  unipotent  characters
constituents  of $R_\bT^\bG(\hbox{Id})$, and in the first column the number
of  these characters  in the  list of  unipotent characters.  In the column
'WGLname' the name of the character $\phi$ of $W_\bG(\bT)$ corresponding to
the unipotent character $\gamma_\phi$ is shown; in the column 'eps' we show
the  sign  $(-1)^{n_\phi}$.  Finally  in  the  last column we show in which
family of unipotent characters is $\gamma_\phi$.
...
|    gap> W:=ComplexReflectionGroup(4);
    ...
    E(3)-series R^{G4}_{(q-1)(q-E3)}() W_G(L,c)=Z6
      |'\|'|    Name WGLname(mod 3) eps  param family #
...
the  torus of type $(q-1)(q-E(3))$.  For cyclic groups $W_\bG(\bL,\lambda)$
we  display  the  parameters  in  the  table  since  they are associated to
characters  of $W_\bG(\bL,\lambda)$.  Finally the  mention '(mod  3)' which
appears in the 'WGLname' column means that in this case the axioms leave an
ambiguity  in the correspondence between unipotent characters $\gamma_\phi$
and characters $\phi$ (as well as with parameters)\:\ the correspondence is
known  only up to  a translation by  3 (in this  case, the same as a global
multiplication of all $\phi$ by $-1$).
...
If the reflection coset or group <W> corresponds to the algebraic group $\bG$
and <cuspidal> to the $d$-cuspidal unipotent character $\lambda$ of $\bL$,
constructs the $d$-series corresponding to $R_\bL^\bG(\lambda)$.
...
's.cuspidal': the index of $\lambda$ in 'UnipotentCharacters(L)'.

's.d': the value of 'd' (represents a root of unity as a fraction in $[0,1[$).

's.WGL': the group $W_\bG(\bL,\lambda)$.

's.RLG': the 'UnipotentCharacter' given by $R_\bL^\bG(\lambda)$.

's.eps':  for each character $\phi$ of  's.WGL' the sign $(-1)^{n_\phi}$ in
the  cohomology  of  the  variety  defining  's.RLG'  of  the corresponding
constituent $\gamma_\phi$ of 's.RLG'.

'Degree(s)': the generic degree of 's.RLG', as a 'CycPol'.

'CharNumbers(s)':   the   indices   in   'UnipotentCharacters(W)'   of  the
constituents of 's.RLG'.

'Hecke(s)': the hecke algebra $H_\bG(\bL,\lambda)$.
