...
def coxi1(arg):
  ...
  m[1][2-1]=-type_
  m[2][1-1]=GAPDiv(2+ER(bond),m[1][2-1])
  return m
...
def coxi8(m):
  ...
  if m%2==0 :
    res["extRefl"]=[1,5,4]
    res["charparams"].extend([[1,GAPDiv(m,2),"'"],[1,GAPDiv(m,2),"''"]])
  else:
    res["extRefl"]=[1,3,2]
  res["charparams"].append([1,m])
  res["charparams"].extend(map(lambda i: [2,i],range(1,QuoInt(m-1,2)+1)))
...
def coxi14(m):
  ...
  if m%2==0 :
    ...
    cl.extend(GAPMul(range(1,GAPDiv(m,2)-1+1),0)+2)
    cl.append(1)
  else:
    cl=[1,m]
    cl.extend(GAPMul(range(1,GAPDiv(m-1,2)+1),0)+2)
  return {"classtext":r,
    "classnames":clnp,
    "classparams":clnp,
...
def coxi18(m,param,rootparam,i):
  if i==1 :
    return [[[param[1][1-1]]],[[param[2][1-1]]]]
  if m%2==0 :
    i=i-2
  if i==0 :
    return [[[param[1][1-1]]],[[param[2][2-1]]]]
  else:
    if i==1 :
      return [[[param[1][2-1]]],[[param[2][1-1]]]]
    else:
      if i==2 :
        return [[[param[1][2-1]]],[[param[2][2-1]]]]
      else:
        u=GAPDiv(-param[1][1-1],param[1][2-1])
        v=GAPDiv(-param[2][1-1],param[2][2-1])
        if !=(m%2,0) :
        ...
        return [GAPMul(-[[-u**0,u**0],[GAPMul(0,u),u]],param[1][2-1]),GAPMul(-[[v,GAPMul(0,v)],[u+v,-v**0]],param[2][2-1])]
...
def coxi25(arg):
  if arg[1-1]%2==0 and !=(arg[3][1-1],arg[3][2-1]) :
    Error(" !  implemented")
...
def coxi30(e):
  ...
  f=f+1-e%2
  uc["harishChandra"].extend(map(lambda x: {"relativeType":{"series":"A",
    "indices":[],
    "rank":0},
    "parameterExponents":[],
    "levi":[1,2],
