...
ChevieData["B"]["CartanMat"]=ylbc1

def ylbc2(r,indices,title,type_):
  print title," ",
  if type_==1 :
...
ChevieData["B"]["PrintDiagram"]=ylbc2
...
ChevieData["B"]["ReflectionName"]=ylbc3
...
ChevieData["B"]["GeneratingRoots"]=ylbc4

def ylbc5(l,s):
  return ChevieData["imp"]["ParabolicRepresentatives"](2,1,l,s)

ChevieData["B"]["ParabolicRepresentatives"]=ylbc5
...
ChevieData["B"]["Size"]=ylbc6

def ylbc7(arg):
  return NrPartitionTuples(arg[1-1],2)

ChevieData["B"]["NrConjugacyClasses"]=ylbc7
...
ChevieData["B"]["ClassParameter"]=ylbc11
...
ChevieData["B"]["LowestPowerFakeDegree"]=ylbc13
...
ChevieData["tmp"]["irreducibles"]=[[ylbc15]]
...
def CHEVIE.tmp.matrix(nq):
  ...
  def hooks(beta,m):
    ...
    cbs.extend(map(lambda x: [[],[x]],prs[2-1]))
    for hk in cbs:
      for pr in prs[1-1]:
...
def ylbc22(l,p):
  ...
  if p==2 :
    return [[range(1,len(pt)+1),GAPMul(map(ylbc23,pt),decS(l))]]
  else:
...
def ylbc23(p):
  ...
  return map(ylbc24,pp[l+1-1])

ChevieData["B"]["DecompositionMatrix"]=ylbc22
...
ChevieData["B"]["UnipotentCharacters"]=ylbc25

def ylbc26(r,type_,char):
  def part2dynkin(part):
    ...
    if type_==1 :
      ...
      res=[p[1-1]]
    res.extend([p[k-1] for k in range(2,len(p)+1)]-[p[k-1] for k in range(1,len(p)-1+1)])
    return res
  ...
  def addSpringer(s):
    ...
    ss.locsys[p-1]=[len(uc["classes"]),CharParams(cc["Au"]).index(map(ylbc27,s["Au"]))+1]
  ...
  for cl in ss:
    ...
    if !=(char,2) :
      ...
      cc["name"]=Join(map(ylbc29,Reversed(Collected(cc.parameter[1-1]))),"")
    cc["red"]=CoxeterGroup()
    if char==2 :
      j=cc.parameter[1-1]
    else:
    ...
    for s in cl:
      addSpringer(s)
  uc["orderClasses"]=Hasse(Poset(map(lambda x: map(ylbc30,uc["classes"]),uc["classes"])))
      ...
      while <=(i,len(p)):
        ...
        if 1==l%4 :
          ...
          if 3==l%4 :
            ...
            a.extend(j+GAPDiv(l+l%4,4)-t)
            b.extend(j+GAPDiv(l-l%4,4))
