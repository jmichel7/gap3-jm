...
ChevieData["D"]["CartanMat"]=eyld1

def eyld2(arg):
  return GAPMul(2**arg[1-1]-1,Factorial(arg[1-1]))

ChevieData["D"]["Size"]=eyld2

def eyld3(r,indices,title):
...
ChevieData["D"]["PrintDiagram"]=eyld3
...
ChevieData["D"]["GeneratingRoots"]=eyld4
...
ChevieData["D"]["WeightInfo"]=eyld5

def eyld6(l,s):
  return ChevieData["imp"]["ParabolicRepresentatives"](2,2,l,s)

ChevieData["D"]["ParabolicRepresentatives"]=eyld6
...
ChevieData["D"]["WordsClassRepresentatives"]=eyld7

def eyld8(n):
  res=ChevieData["imp"]["ClassInfo"](2,2,n)
  res["classparams"]=map(eyld9,res["classparams"])
...
ChevieData["D"]["NrConjugacyClasses"]=eyld10
...
ChevieData["D"]["ClassParameter"]=eyld12
...
ChevieData["tmp"]["irreducibles"]=[[eyld13]]
...
ChevieData["D"]["FactorizedSchurElement"]=eyld14
...
ChevieData["D"]["HeckeRepresentation"]=eyld15
...
ChevieData["D"]["UnipotentCharacters"]=eyld22
...
def eyld23(n,char):
  def addSpringer(s,i):
    ...
    ss.locsys[p-1]=[i,CharParams(cc["Au"]).index(map(eyld24,s["Au"]))+1]
  ...
  uc={"classes":[],
    "springerSeries":map(eyld25,l)}
  for cl in ss:
    cc={"parameter":symbol2partition(cl[1-1]["symbol"])}
    if char==2 :
      cc["dimBu"]=cl[1-1]["dimBu"]
      cc["name"]=Join(map(eyld26,Reversed(Collected(cc.parameter[1-1]))),"")
    else:
    ...
    if !(IsList(cl[1].sp[2-1])) :
      ...
      for s in cl:
        addSpringer(s,len(uc["classes"]))
  if char==2 :
    uc["orderClasses"]=Hasse(Poset(map(lambda x: map(eyld27,uc["classes"]),uc["classes"])))
  else:
...
ChevieData["D"]["UnipotentClasses"]=eyld23
